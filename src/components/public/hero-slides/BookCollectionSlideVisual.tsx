...
export default function BookCollectionSlideVisual({
  ...
  return (
    <div className="relative min-h-[85vh] w-full overflow-hidden bg-hero-cream">
      <DecorativeHeroBackground variant="books" />
      <div className="absolute inset-0 bg-gradient-to-r from-hero-cream/95 via-hero-cream/60 to-white/20" />
      ...
      <div className="relative z-10 mx-auto grid min-h-[85vh] w-full max-w-[1600px] items-center gap-5 px-12 pb-24 pt-8 sm:gap-7 sm:px-16 sm:pt-10 lg:grid-cols-[minmax(260px,0.7fr)_minmax(0,1.3fr)] lg:gap-8 lg:px-20 lg:py-16 xl:grid-cols-[minmax(340px,0.68fr)_minmax(680px,1.32fr)] xl:px-24">
        ...
        >
          {books.map((book, index) => (
            <figure
              key={book.id}
              className={`absolute left-1/2 top-1/2 aspect-[2/3] shrink-0 overflow-hidden rounded-xl border border-white bg-surface shadow-[0_22px_50px_rgba(44,44,44,0.18)] ring-1 ring-primary/10 transition-transform duration-300 lg:static lg:left-auto lg:top-auto lg:z-auto lg:block lg:w-[clamp(8.5rem,11vw,12.5rem)] lg:translate-x-0 lg:translate-y-0 ${
